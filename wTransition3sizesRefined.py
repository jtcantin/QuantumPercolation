...
data2DArray_Nw_srtd_indices = np.lexsort(np.vstack((wArray,NArray)))

data2DArray_Nw_srtd = data2DArray[:,data2DArray_Nw_srtd_indices]
#print data2DArray_Nw_srtd
...
while newNIndex < data2DArray_Nw_srtd[0].size:

    oldNIndex = newNIndex
    NvalueCurrent = data2DArray_Nw_srtd[0,oldNIndex]
    newNIndex = np.sum(data2DArray_Nw_srtd[0] <= data2DArray_Nw_srtd[0,newNIndex])
#    print newNIndex

    relevant_data2DArray_Nw_srtd = data2DArray_Nw_srtd[:, oldNIndex:newNIndex]
    
#    print relevant_data2DArray_Nw_srtd
    ...
    while newWIndex < relevant_data2DArray_Nw_srtd[2].size:

        oldWIndex = newWIndex
        wPlotList.append(relevant_data2DArray_Nw_srtd[2,oldWIndex])
        
        newWIndex += np.sum(np.abs(relevant_data2DArray_Nw_srtd[2] - relevant_data2DArray_Nw_srtd[2,newWIndex]) < 1E-10)
#        print newWIndex

        relevant_bArray = relevant_data2DArray_Nw_srtd[3,oldWIndex:newWIndex]
...
#print data2DArray_Nw_srtd[0] <= data2DArray_Nw_srtd[0,0]
#print np.sum(data2DArray_Nw_srtd[0] <= data2DArray_Nw_srtd[0,0]) #This gives next index after first value
#print data2DArray_Nw_srtd[:,data2DArray_Nw_srtd[0] <= data2DArray_Nw_srtd[0,0]]
...
#yerr=errorBarArray
#    line = ax.errorbar(wPlotArray, bPlotArray, yerr=errorBarArray, marker=markerList[typeIndex], markersize=markerSizeList[typeIndex], label="N={0:d}".format(int(NvalueCurrent)), clip_on=False, linewidth=1.0, color=colourList[typeIndex], linestyle=LineStyleList[typeIndex])#linewidth=3.0, ls=''
#    line = ax.plot(wPlotArray, bPlotArray, marker=markerList[typeIndex], markersize=markerSizeList[typeIndex], label="N={0:d}".format(int(NvalueCurrent)), clip_on=False, linewidth=3.0, color=colourList[typeIndex], linestyle=LineStyleList[typeIndex])#linewidth=3.0, ls=''
#    line = ax.plot(wPlotArray, bPlotArray, marker=markerList[typeIndex], markersize=markerSizeList[typeIndex], label="N={0:d}".format(int(NvalueCurrent)), clip_on=False, linewidth=3.0, color=colourList[typeIndex], linestyle="None")#linewidth=3.0, ls=''
...
figureFilename = "wTransitionNN_q25.eps"
