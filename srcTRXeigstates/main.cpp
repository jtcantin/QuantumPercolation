...
//        cout << eigvals[i] << endl;
    ...
    //Output eigenstates to file
    ofstream myfile3;
    myfile3.open((outfile+"_Estates.txt").c_str());
    myfile3 << setprecision(presInt);
    myfile3 << "# Site label, C_1, C_2, ... C_N where _? refers to eigenstate label." << endl;
    myfile3 << "# Site label, l, defined as l = x+Nx*(y+Ny*z) for point (x,y,z)" << endl;
    cout << "Eigenstates recording..." << endl;
    
    int i,j;
    for (i=0; i<occnum; i++) {
        myfile3 << i << ": ";
        for (j=0; j<numEigvals; j++) {
            myfile3 << eigStateMatrix[i + occnum*j] << " ";
        }
        myfile3 << endl;
    }
    myfile3.close();
